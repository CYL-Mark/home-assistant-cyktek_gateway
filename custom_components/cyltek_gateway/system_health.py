...
async def system_health_info(hass: HomeAssistant):
    ...
    info = {"version": f"{integration.version} ({util.source_hash(os.path.join(__file__))})"}
...
async def setup_debug(hass: HomeAssistant, logger: Logger):
    ...
    info[DOMAIN + "_version"] = f"{integration.version} ({util.source_hash(os.path.join(__file__))})"
